...
def test_cancel_pending_leaf_task_succeeds(tasks_root: Path, story_id: str) -> None:
...
def test_cancel_leaf_task_parses_as_cancelled(tasks_root: Path, story_id: str) -> None:
    task_id = add_subtask(story_id, "Leaf task").task_id
    assert_invoke(app, ["cancel", task_id])
    task_file = next(tasks_root.glob(f"{story_id}-*.md"))
...
def test_cancel_already_cancelled_succeeds(tasks_root: Path, story_id: str) -> None:
    task_id = add_subtask(story_id, "Leaf task").task_id
    assert_invoke(app, ["cancel", task_id])
    result = assert_invoke(app, ["cancel", task_id])
...
def test_cancel_in_progress_task(tasks_root: Path, story_id: str) -> None:
    task_id = add_subtask(story_id, "Leaf task").task_id
...
def test_cancel_done_task(tasks_root: Path, story_id: str) -> None:
    task_id = add_subtask(story_id, "Leaf task").task_id
...
def test_cancel_task_with_subtasks_fails(tasks_root: Path, story_id: str) -> None:
...
def test_cancel_nonleaf_hints_force(tasks_root: Path, story_id: str) -> None:
    add_subtask(story_id, "Subtask one")
    result = assert_invoke(app, ["cancel", story_id], expect_error=True)
    assert "--force" in result.output


def test_cancel_force_succeeds_with_open_subtasks(
    tasks_root: Path, story_id: str
) -> None:
    add_subtask(story_id, "Subtask one")
    add_subtask(story_id, "Subtask two")
...
def test_cancel_force_marks_parent_cancelled(tasks_root: Path, story_id: str) -> None:
    add_subtask(story_id, "Subtask one")
    add_subtask(story_id, "Subtask two")
    assert_invoke(app, ["cancel", "--force", story_id])
    task_file = next(tasks_root.glob(f"{story_id}-*.md"))
...
def test_cancel_force_preserves_done_subtasks(tasks_root: Path, story_id: str) -> None:
    t01 = add_subtask(story_id, "Subtask one").task_id
    add_subtask(story_id, "Subtask two")
    assert_invoke(app, ["done", t01])
    assert_invoke(app, ["cancel", "--force", story_id])
...
def test_cancel_force_on_leaf_task_works(tasks_root: Path, story_id: str) -> None:
    task_id = add_subtask(story_id, "Leaf task").task_id
...
def test_done_force_skips_cancelled_subtasks(tasks_root: Path, story_id: str) -> None:
    t01 = add_subtask(story_id, "Subtask one").task_id
    t02 = add_subtask(story_id, "Subtask two").task_id
    assert_invoke(app, ["cancel", t01])
...
def test_json_cancel_outputs_task_ref(tasks_root: Path, story_id: str) -> None:
    task_id = add_subtask(story_id, "Leaf task").task_id
...
def test_cancel_already_cancelled_nonleaf_succeeds(
    tasks_root: Path, story_id: str
) -> None:
    add_subtask(story_id, "Subtask one")
    assert_invoke(app, ["cancel", "--force", story_id])
