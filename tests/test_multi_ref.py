...
def test_start_multiple_tasks(story_id: str) -> None:
    ...
    result = assert_invoke(app, ["start", f"{story_id}t01", f"{story_id}t02"])
...
def test_start_multiple_updates_disk(story_id: str) -> None:
    assert_invoke(app, ["add", story_id, "Task one"])
    assert_invoke(app, ["add", story_id, "Task two"])
    assert_invoke(app, ["start", f"{story_id}t01", f"{story_id}t02"])
...
def test_done_multiple_tasks(story_id: str) -> None:
    assert_invoke(app, ["add", story_id, "Task one"])
    assert_invoke(app, ["add", story_id, "Task two"])
    result = assert_invoke(app, ["done", f"{story_id}t01", f"{story_id}t02"])
    assert f"{story_id}t01" in result.output
    assert f"{story_id}t02" in result.output
...
def test_done_multiple_updates_disk(story_id: str) -> None:
    assert_invoke(app, ["add", story_id, "Task one"])
    assert_invoke(app, ["add", story_id, "Task two"])
    assert_invoke(app, ["done", f"{story_id}t01", f"{story_id}t02"])
    task_file = next(Path("planning").glob(f"{story_id}-*.md"))
    task = parse_task_file(task_file)
...
def test_cancel_multiple_tasks(story_id: str) -> None:
    assert_invoke(app, ["add", story_id, "Task one"])
    assert_invoke(app, ["add", story_id, "Task two"])
    result = assert_invoke(app, ["cancel", f"{story_id}t01", f"{story_id}t02"])
    assert f"{story_id}t01" in result.output
    assert f"{story_id}t02" in result.output
...
def test_cancel_multiple_updates_disk(story_id: str) -> None:
    assert_invoke(app, ["add", story_id, "Task one"])
    assert_invoke(app, ["add", story_id, "Task two"])
    assert_invoke(app, ["cancel", f"{story_id}t01", f"{story_id}t02"])
    task_file = next(Path("planning").glob(f"{story_id}-*.md"))
    task = parse_task_file(task_file)
...
def test_reset_multiple_tasks(story_id: str) -> None:
    ...
    result = assert_invoke(app, ["reset", f"{story_id}t01", f"{story_id}t02"])
    assert f"{story_id}t01" in result.output
    assert f"{story_id}t02" in result.output
...
def test_reset_multiple_updates_disk(story_id: str) -> None:
    ...
    assert_invoke(app, ["start", f"{story_id}t01"])
    assert_invoke(app, ["start", f"{story_id}t02"])
    assert_invoke(app, ["reset", f"{story_id}t01", f"{story_id}t02"])
    task_file = next(Path("planning").glob(f"{story_id}-*.md"))
    task = parse_task_file(task_file)
...
def test_json_start_multiple(story_id: str) -> None:
    ...
    result = assert_invoke(
        app, ["--json-output", "start", f"{story_id}t01", f"{story_id}t02"]
    )
    data = json.loads(result.output)
    assert data["task_refs"] == [f"{story_id}t01", f"{story_id}t02"]
...
def test_json_done_multiple(story_id: str) -> None:
    ...
    assert_invoke(app, ["add", story_id, "Task two"])
    result = assert_invoke(
        app, ["--json-output", "done", f"{story_id}t01", f"{story_id}t02"]
    )
    data = json.loads(result.output)
    assert data["task_refs"] == [f"{story_id}t01", f"{story_id}t02"]
