...
def test_start_leaf_task_parses_as_in_progress(story_id: str, tasks_root: Path) -> None:
    task_id = add_subtask(story_id, "Leaf task").task_id
    assert_invoke(app, ["start", task_id])
    task_file = next(tasks_root.glob(f"{story_id}-*.md"))
...
def test_restart_done_task(story_id: str, tasks_root: Path) -> None:
...
def test_start_subtask_sets_parent_in_progress(story_id: str, tasks_root: Path) -> None:
    task_id = add_subtask(story_id, "Leaf task").task_id
    assert_invoke(app, ["start", task_id])
    task_file = next(tasks_root.glob(f"{story_id}-*.md"))
    task = parse_task_file(task_file).task
